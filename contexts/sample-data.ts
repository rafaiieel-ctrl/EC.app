...
export const SAMPLE_QUESTIONS: Omit<Question, 'id'>[] = sampleDataText
  ...
  .map(line => {
    ...
    const [
      date, bank, position, subject, topic, qRef, qText,
    ...
    const masteryScore = (lastWasCorrect ? 70 : 40) + (selfEvalLevel * 5) - (Math.random() * 10);
    
    // Fake some SRS progress
    const srsStage = Math.floor(Math.random() * 3);
    const nextReviewDays = [0.5, 1, 7, 14, 30][srsStage];
    const nextReviewDate = srs.addDaysISO(srs.todayISO(), (Math.random() * 14 - 7)); // Spread reviews around today
    ...
    return {
      ...
      explanation: expl,
      comments: '', // Initialize comments for sample questions
      yourAnswer: yAns,
      correctAnswer: cAns,
      ...
      isCritical: Math.random() < 0.1, // Randomly mark some as critical
    };
  });